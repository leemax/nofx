...
-   **前端**：
    ...
    -   ✅ **错误处理**：增加了切换状态失败时的用户提示。

### 📈 功能改进：市场数据增强

-   **后端**：
    -   ✅ **3M 布林带**：在3分钟K线数据中增加了20周期、2.0标准差的布林带上轨和下轨计算。
    -   ✅ **3M ATR**：在3分钟K线数据中增加了14周期ATR的计算。
    -   ✅ **15M EMA-50**：新增了15分钟K线数据的50周期EMA计算。
    -   ✅ **AI快照更新**：所有新增的市场数据指标均已集成到发送给AI的决策快照中。

### 🛠️ Bug修复

-   ✅ **前端UI修复**：修复了前端页面同时显示两个运行开关的UI问题。
-   ✅ **提示词选择器**：修复了提示词选择下拉菜单消失的问题。
-   ✅ **开关响应**：解决了运行开关响应不及时，状态显示不正确的问题。
-   ✅ **数据库错误修复**：解决了WebSocket接收交易数据时，因 `trade_id` 重复导致的 `UNIQUE constraint failed` 数据库插入错误。
-   ✅ **夏普比率计算修复**：解决了在计算夏普比率时，因数据记录不足可能导致的 `integer divide by zero` 运行时错误。
